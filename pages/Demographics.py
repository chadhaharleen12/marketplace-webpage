...
# Add a selection for the user to choose a webpage (mapping PAGE_NAME to an option)
# page_name = st.selectbox("Select a webpage:", ['fer', 'banking', 'covid', 'demographics'])

# Query the table to fetch data for the selected page
query = f"""
SELECT PAGE_NAME, OVERVIEW, VIEWS_PUBLISHED, SOURCE_LINK, IMAGE_URLS
FROM WEBPAGE_DATA 
WHERE PAGE_NAME = 'Demographics'
"""
...
if not df.empty:
    ...
    # st.write(f"**Overview of {page_name.capitalize()} webpage:**")
